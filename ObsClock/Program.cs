...
namespace ObsClock
{
    class Program
    {
        ...
        private static SettingDataModel? Settings { get; set; }
            ...
            static void SetTimer()
            {
                ClockTimer
                    ...
                    .Subscribe(_ =>
                    {
                        ...
                        Console.WriteLine($"次の送出時刻: {_nextShowTime:MM/dd HH:mm:ss}");
                    });
            }
        }
        ...
        {
            Console.WriteLine($"{Settings?.Url} に接続しました");

            _textItem = GetTextItem(Settings?.TextSourceName);
            if (_textItem == default)
            {
                throw new Exception($"オブジェクト \'{Settings?.TextSourceName}\' が見つかりませんでした");
            }
